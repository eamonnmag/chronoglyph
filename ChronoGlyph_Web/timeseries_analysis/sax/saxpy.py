...
class SAX(object):
    ...
    def __init__(self, wordSize=2, alphabetSize=4, epsilon=1e-6, eSAX=False, tSAX=False, fSAX=False):
    ...
    def to_PAA(self, x):
        ...
        while frameStart <= n - step:
            ...
            # this part adds in the lower and upper bounds for the section in consideration
            # as proposed in the Extended SAX specification. The order should be dictated by the origin of the
            # value in the series. e.g. if the max occurs before the mean, this should be first. If it comes after, it
            # should be last.
            if self.eSAX:
                self.to_PAA_eSAX_extension(thisFrame, approximation)
            elif self.tSAX:
                self.to_PAA_tSAX_extension(thisFrame, approximation)
            elif self.fSAX:
                self.to_PAA_fSAX_extension(thisFrame, approximation)
            else:
    ...
    def to_PAA_tSAX_extension(self, frame, approximation):
        """
        Adds the the fSAX (feature SAX) values to the approximation data structure where they should appear in the series
        """
        mean = np.mean(frame)
        min = np.min(frame)
        max = np.max(frame)

        approximation.append(min)
        approximation.append(mean)
        approximation.append(max)

    def to_PAA_fSAX_extension(self, frame, approximation):
        """
        Adds the the fSAX (feature SAX) values to the approximation data structure where they should appear in the series
        """
        mean = np.mean(frame)
        ...
        # tolerance level before considering min and max different
        print min, ",", max
        if min > max:
            approximation.append(max)
        elif min < max:
    ...
    def to_PAA_eSAX_extension(self, frame, approximation):
        """
        Adds the the eSAX values to the approximation data structure where they should appear in the series.
        """
        mean = np.mean(frame)
        min = np.min(frame)
        max = np.max(frame)
        ...
        # print 'min value is ' + str(min) + ' at Index ' + str(minIndex)
        #print 'max value is ' + str(max) + ' at Index ' + str(maxIndex)
        #print 'mean value is ' + str(mean) + ' at Index ' + str(meanIndex)
...
class DictionarySizeIsNotSupported(Exception):
    pass


class StringsAreDifferentLength(Exception):
    pass


class OverlapSpecifiedIsNotSmallerThanWindowSize(Exception):
    pass
